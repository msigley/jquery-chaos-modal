...
!1);g.each(function(){var b=a(this),d=b.data("src");if(d){if(c.iframeAddAutoplay){var e=d.indexOf("?");-1==e?d+="?":e!=d.length-1&&(d+="&amp;");d+="autoplay=1"}b.attr("src",d).attr("data-src","")}});b.prependTo(e);if(l){a.chaosModalPrintCSSElements||(a.chaosModalPrintCSSElements=a(document.head).find('link[rel="stylesheet"]'));e=a('<iframe class="print-iframe" width="100" height="100" style="position: absolute; left: -20000px; top: 0;"></iframe>');b.append(e);var k=e.get(0).contentDocument;k.open();
k.write("<!DOCTYPE html>");k.write("<html><head>");a.chaosModalPrintCSSElements.each(function(){k.write(this.outerHTML)});k.write("</head><body>");k.write(n);k.write("</body></html>");k.close()}var f=function(){b.width()>a.chaosModalMaxWidth?b.width(a.chaosModalMaxWidth):b.width(b.width());b.css("max-width","92%");b.find("img").css("max-width","100%");b.hide();b.resizeModal(q);0<b.find(".print-link").length&&b.find(".print-link").bind("click",r);0<b.find(".close-link").length&&b.find(".close-link").bind("click",
...
function(c){var b=a(this);c.preventDefault();b.off("click",a.processModalLink);var e=b.data("chaos-modal-box-id"),d=c=!1,g=/\.(jpeg|jpg|gif|png|bmp|wbmp)$/i,f=!1;c=e?a("#"+e).first():b.parent().find(".chaos-modal-box").first();c.length?(d=a("<div></div>").css({padding:"20px"}),c.clone(!0).css({display:"block"}).appendTo(d),d=a("<div></div>").css({background:"#fff"}).append(d)):(g.test(b.attr("href"))?f=b.attr("href"):(e=b.find("img"),1==e.length&&g.test(e.attr("src"))&&(f=e.attr("src"))),f&&(d=a("<div></div>").css({padding:"20px"}),
a('<img src="'+f+'" />').css({display:"block"}).appendTo(d),d=a("<div></div>").css({background:"#fff"}).append(d)));d&&(c.remove(),b.data("chaos-modal-href",b.attr("href")),b.data("chaos-modal-target",b.attr("target")),b.attr({href:"#",target:null,id:"chaos-modal-link-"+a.chaosModalIndex}),d.attr({id:"chaos-modal-box-"+a.chaosModalIndex}),d.hide(),d.appendTo("body"),b.click(function(){a.fn.showLoading();var c={maxWidth:960,closeLink:!0,printLink:!1,alwaysAtTop:!1,preprocessing:!1,clickPassthrough:!1,
iframeAddAutoplay:!0};a.each(c,function(a,d){var e=b.data("chaos-modal-"+a);"undefined"!==typeof e&&(c[a]="maxWidth"==a?parseInt(e):e)});c.clickPassthrough&&(c.clickPassthrough=b.data("chaos-modal-href"),c.clickTarget=b.data("chaos-modal-target"),c.clickTarget||(c.clickTarget="_self"));preprocess=b.attr("data-preprocess");var d=a(this).attr("id").match(/chaos-modal-link-(\d+)/i);d[1]&&(d=a("#chaos-modal-box-"+d[1]),1==d.length&&"true"!=preprocess?d.openModal(c):1==d.length&&"true"==preprocess&&b.trigger("chaos-modal-preprocess",
d));return!1}),b.click(),a.chaosModalIndex++);return!1}})(jQuery);
